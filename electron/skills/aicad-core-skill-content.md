...
- **API lookup (error-only)**: do **not** call `api -module build123d` while planning or before a build attempt. The desktop bridge blocks api until `rebuild_model` or `script build` has failed. After a failure, use `api -module build123d -search <keyword>` or `api -module build123d -name <Symbol>` only to resolve API/signature mistakes in stderr — never a bare `api -module build123d` list. For selectors and topology use `probe`; for project patterns use `grep` and the build123d skill.
