...
function createMainUiTools({
  ...
  const menuMessages = {
    ...
    'zh-CN': {
      ...
      updateNotAvailableMessage: '当前已是最新版本',
      updateAvailableTitle: '发现新版本',
      updateAvailablePromptMessage: '检测到 Forgent3D {version}，是否立即下载？',
      updateAvailablePromptDetail: '下载完成后可立即重启安装，也可在下次退出时自动安装。',
      updateDownloadNow: '下载',
      updateDownloadingTitle: '正在下载更新',
      updateDownloadingLabel: '正在下载 Forgent3D {version}…',
  ...
  function getUpdateMessages() {
    return {
      ...
      updateAvailablePromptMessage: t('updateAvailablePromptMessage'),
      updateAvailablePromptDetail: t('updateAvailablePromptDetail'),
      updateDownloadNow: t('updateDownloadNow'),
      updateDownloadingTitle: t('updateDownloadingTitle'),
      updateDownloadingLabel: t('updateDownloadingLabel'),
