...
- **Feature Tags**: For important functional topology, import `tag_feature` from `aicad_select` and tag after final `result` exists, e.g. `tag_feature(metadata, "mounting_holes", faces=holes(result, radius=3, axis=Axis.Z), selector="holes(radius=3, axis=Axis.Z)", kind="hole")`. Write `selector` in the viewer's copy form — omit the part argument (not `holes(part, ...)`). Tags are metadata selector hints, not persistent BREP ids; do not tag every generic face.
